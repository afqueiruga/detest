...
class SquarePoisson():
    ...
    def __call__(self, xt):
        ...
        t = lambda k, x, y: np.sin(k * np.pi * (1.0 + x) / 2.0) / (
            k * k * k * np.sinh(k * np.pi)) * (np.sinh(k * np.pi * (
                1.0 + y) / 2.0) + np.sinh(k * np.pi * (1.0 - y) / 2.0))
        # TODO is there a mistake here? yt?
        for i, y in enumerate(X):
            panal[i] = (1.0 - xt[0]**2.0) / 2.0 - 16.0 / (np.pi)**3 * sum(
                [t(k, yt[0], yt[1]) for k in xrange(1, 101, 2)])
        return {'P': panal}
