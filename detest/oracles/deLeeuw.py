...
class DeLeeuw():
    ...
    def __init__(self, in_params=None):
        ...
        m = 0.5 * (K_d + 4.0 * G / 3.0) / (2.0 *
                                           G) * (alpha**2 + S *
                                                 (K_d + G / 3.0)) / (alpha**2)
        # c  = ( k/eta )/(  S+alpha**2/(K_d+4.0/3.0*G)  )
        c = (k / eta) * (K_d + 4.0 * G / 3.0) / (alpha**2 + S *
                                                 (K_d + 4.0 * G / 3.0))
        ...
        for j in range(600):
            last = xi_j[-1]
            new = nsolve(g,
                         xi, (last + 1.0e-4, last + (1.2 * pi.evalf())),
                         solver='bisect')
        ...
        def P_sci(r, t):
            return sum([(p0 * term_scipy(r, t, i))
                        for i in range(len(xi_j))]) + params['P_background']

        def U(r, t):
            # TODO This is not 100% right. Put in the u solution. Is there one?
